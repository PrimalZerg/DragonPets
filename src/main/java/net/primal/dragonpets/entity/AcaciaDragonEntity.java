...
public class AcaciaDragonEntity extends TamableAnimal {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MONSTER)
					.add(new MobSpawnSettings.SpawnerData(DragonPetsModEntities.ACACIA_DRAGON.get(), 120, 1, 1));
	}

	public AcaciaDragonEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(DragonPetsModEntities.ACACIA_DRAGON.get(), world);
	}

	public AcaciaDragonEntity(EntityType<AcaciaDragonEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(3, new Goal() {
			...
			public boolean canUse() {
				if (AcaciaDragonEntity.this.getTarget() != null && !AcaciaDragonEntity.this.getMoveControl().hasWanted()) {
					return true;
				} else {
					return false;
				}
			...
			public boolean canContinueToUse() {
				return AcaciaDragonEntity.this.getMoveControl().hasWanted() && AcaciaDragonEntity.this.getTarget() != null
						&& AcaciaDragonEntity.this.getTarget().isAlive();
			}

			@Override
			public void start() {
				LivingEntity livingentity = AcaciaDragonEntity.this.getTarget();
				Vec3 vec3d = livingentity.getEyePosition(1);
				AcaciaDragonEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
			}

			@Override
			public void tick() {
				LivingEntity livingentity = AcaciaDragonEntity.this.getTarget();
				if (AcaciaDragonEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
					AcaciaDragonEntity.this.doHurtTarget(livingentity);
				} else {
					double d0 = AcaciaDragonEntity.this.distanceToSqr(livingentity);
					if (d0 < 16) {
						Vec3 vec3d = livingentity.getEyePosition(1);
						AcaciaDragonEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
					}
		...
		this.goalSelector.addGoal(11, new RandomStrollGoal(this, 0.8, 20) {
			...
			protected Vec3 getPosition() {
				Random random = AcaciaDragonEntity.this.getRandom();
				double dir_x = AcaciaDragonEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = AcaciaDragonEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = AcaciaDragonEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		AcaciaDragonEntity retval = DragonPetsModEntities.ACACIA_DRAGON.get().create(serverWorld);
	...
	public boolean isFood(ItemStack stack) {
		return List.of(Items.SWEET_BERRIES, Items.BEEF, Items.CHICKEN, Items.PORKCHOP, Items.MUTTON).contains(stack.getItem());
	}
