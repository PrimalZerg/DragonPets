...
public class EndDragonEntity extends TamableAnimal {
	...
	public EndDragonEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(DragonPetsModEntities.END_DRAGON.get(), world);
	}

	public EndDragonEntity(EntityType<EndDragonEntity> type, Level world) {
		super(type, world);
		xpReward = 3;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(6, new Goal() {
			...
			public boolean canUse() {
				if (EndDragonEntity.this.getTarget() != null && !EndDragonEntity.this.getMoveControl().hasWanted()) {
					return true;
				} else {
					return false;
				}
			...
			public boolean canContinueToUse() {
				return EndDragonEntity.this.getMoveControl().hasWanted() && EndDragonEntity.this.getTarget() != null
						&& EndDragonEntity.this.getTarget().isAlive();
			}

			@Override
			public void start() {
				LivingEntity livingentity = EndDragonEntity.this.getTarget();
				Vec3 vec3d = livingentity.getEyePosition(1);
				EndDragonEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
			}

			@Override
			public void tick() {
				LivingEntity livingentity = EndDragonEntity.this.getTarget();
				if (EndDragonEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
					EndDragonEntity.this.doHurtTarget(livingentity);
				} else {
					double d0 = EndDragonEntity.this.distanceToSqr(livingentity);
					if (d0 < 16) {
						Vec3 vec3d = livingentity.getEyePosition(1);
						EndDragonEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
					}
		...
		this.goalSelector.addGoal(9, new TemptGoal(this, 1, Ingredient.of(Items.GOLDEN_APPLE,Items.CHORUS_FRUIT), false));
		...
		this.goalSelector.addGoal(11, new RandomStrollGoal(this, 0.8, 20) {
			...
			protected Vec3 getPosition() {
				...
				double dir_x = EndDragonEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = EndDragonEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = EndDragonEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		EndDragonEntity retval = DragonPetsModEntities.END_DRAGON.get().create(serverWorld);
	...
	public static void init() {
		SpawnPlacements.register(DragonPetsModEntities.END_DRAGON.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
