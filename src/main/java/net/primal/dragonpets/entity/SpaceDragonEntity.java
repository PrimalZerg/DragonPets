...
public class SpaceDragonEntity extends TamableAnimal {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MONSTER)
					.add(new MobSpawnSettings.SpawnerData(DragonPetsModEntities.SPACE_DRAGON.get(), ConfigHandler.SERVER.SPACE_DRAGON_SPAWN_WEIGHT.get(), 1, 1));
	}

	public SpaceDragonEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(DragonPetsModEntities.SPACE_DRAGON.get(), world);
	}

	public SpaceDragonEntity(EntityType<SpaceDragonEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(3, new Goal() {
			...
			public boolean canUse() {
				if (SpaceDragonEntity.this.getTarget() != null && !SpaceDragonEntity.this.getMoveControl().hasWanted()) {
					return true;
				} else {
					return false;
				}
			...
			public boolean canContinueToUse() {
				return SpaceDragonEntity.this.getMoveControl().hasWanted() && SpaceDragonEntity.this.getTarget() != null
						&& SpaceDragonEntity.this.getTarget().isAlive();
			}

			@Override
			public void start() {
				LivingEntity livingentity = SpaceDragonEntity.this.getTarget();
				Vec3 vec3d = livingentity.getEyePosition(1);
				SpaceDragonEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
			}

			@Override
			public void tick() {
				LivingEntity livingentity = SpaceDragonEntity.this.getTarget();
				if (SpaceDragonEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
					SpaceDragonEntity.this.doHurtTarget(livingentity);
				} else {
					double d0 = SpaceDragonEntity.this.distanceToSqr(livingentity);
					if (d0 < 16) {
						Vec3 vec3d = livingentity.getEyePosition(1);
						SpaceDragonEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
					}
		...
		this.goalSelector.addGoal(10, new RandomStrollGoal(this, 0.8, 20) {
			...
			protected Vec3 getPosition() {
				Random random = SpaceDragonEntity.this.getRandom();
				double dir_x = SpaceDragonEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = SpaceDragonEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = SpaceDragonEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		SpaceDragonEntity retval = DragonPetsModEntities.SPACE_DRAGON.get().create(serverWorld);
	...
	public boolean isFood(ItemStack stack) {
		return List.of(Items.CHORUS_FRUIT).contains(stack.getItem());
	}
	...
	public static void init() {
		SpawnPlacements.register(DragonPetsModEntities.SPACE_DRAGON.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
