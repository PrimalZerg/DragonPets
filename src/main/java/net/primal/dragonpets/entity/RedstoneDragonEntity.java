...
public class RedstoneDragonEntity extends TamableAnimal {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MONSTER)
					.add(new MobSpawnSettings.SpawnerData(DragonPetsModEntities.REDSTONE_DRAGON.get(), ConfigHandler.SERVER.REDSTONE_DRAGON_SPAWN_WEIGHT.get(), 1, 1));
	}

	public RedstoneDragonEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(DragonPetsModEntities.REDSTONE_DRAGON.get(), world);
	}

	public RedstoneDragonEntity(EntityType<RedstoneDragonEntity> type, Level world) {
		super(type, world);
		xpReward = 3;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(5, new Goal() {
			...
			public boolean canUse() {
				if (RedstoneDragonEntity.this.getTarget() != null && !RedstoneDragonEntity.this.getMoveControl().hasWanted()) {
					return true;
				} else {
					return false;
				}
			...
			public boolean canContinueToUse() {
				return RedstoneDragonEntity.this.getMoveControl().hasWanted() && RedstoneDragonEntity.this.getTarget() != null
						&& RedstoneDragonEntity.this.getTarget().isAlive();
			}

			@Override
			public void start() {
				LivingEntity livingentity = RedstoneDragonEntity.this.getTarget();
				Vec3 vec3d = livingentity.getEyePosition(1);
				RedstoneDragonEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
			}

			@Override
			public void tick() {
				LivingEntity livingentity = RedstoneDragonEntity.this.getTarget();
				if (RedstoneDragonEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
					RedstoneDragonEntity.this.doHurtTarget(livingentity);
				} else {
					double d0 = RedstoneDragonEntity.this.distanceToSqr(livingentity);
					if (d0 < 16) {
						Vec3 vec3d = livingentity.getEyePosition(1);
						RedstoneDragonEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
					}
		...
		this.goalSelector.addGoal(10, new RandomStrollGoal(this, 0.8, 20) {
			...
			protected Vec3 getPosition() {
				Random random = RedstoneDragonEntity.this.getRandom();
				double dir_x = RedstoneDragonEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = RedstoneDragonEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = RedstoneDragonEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		RedstoneDragonEntity retval = DragonPetsModEntities.REDSTONE_DRAGON.get().create(serverWorld);
	...
	public static void init() {
		SpawnPlacements.register(DragonPetsModEntities.REDSTONE_DRAGON.get(), SpawnPlacements.Type.ON_GROUND,
