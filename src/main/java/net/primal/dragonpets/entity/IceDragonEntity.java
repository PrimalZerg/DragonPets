...
public class IceDragonEntity extends TamableAnimal {
	private static final Set<ResourceLocation> SPAWN_BIOMES = Set.of(new ResourceLocation("frozen_ocean"), new ResourceLocation("frozen_river"),
			new ResourceLocation("snowy_plains"), new ResourceLocation("windswept_hills"), new ResourceLocation("ice_spikes"),
			new ResourceLocation("snowy_slopes"), new ResourceLocation("frozen_peaks"), new ResourceLocation("snowy_taiga"),
	...
	public IceDragonEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(DragonPetsModEntities.ICE_DRAGON.get(), world);
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(3, new Goal() {
			...
			public boolean canUse() {
				if (IceDragonEntity.this.getTarget() != null && !IceDragonEntity.this.getMoveControl().hasWanted()) {
					return true;
				} else {
					return false;
				}
			...
			public boolean canContinueToUse() {
				return IceDragonEntity.this.getMoveControl().hasWanted() && IceDragonEntity.this.getTarget() != null
						&& IceDragonEntity.this.getTarget().isAlive();
			}

			@Override
			public void start() {
				LivingEntity livingentity = IceDragonEntity.this.getTarget();
				Vec3 vec3d = livingentity.getEyePosition(1);
				IceDragonEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
			}

			@Override
			public void tick() {
				LivingEntity livingentity = IceDragonEntity.this.getTarget();
				if (IceDragonEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
					IceDragonEntity.this.doHurtTarget(livingentity);
				} else {
					double d0 = IceDragonEntity.this.distanceToSqr(livingentity);
					if (d0 < 16) {
						Vec3 vec3d = livingentity.getEyePosition(1);
						IceDragonEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
					}
		...
		this.goalSelector.addGoal(10, new RandomStrollGoal(this, 0.8, 20) {
			...
			protected Vec3 getPosition() {
				Random random = IceDragonEntity.this.getRandom();
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		IceDragonEntity retval = DragonPetsModEntities.ICE_DRAGON.get().create(serverWorld);
	...
	public void aiStep() {
		...
		for (int l = 0; l < 1; ++l) {
			...
			world.addParticle(ParticleTypes.SNOWFLAKE, x0, y0, z0, dx, dy, dz);
		}
	}

	public static void init() {
		SpawnPlacements.register(DragonPetsModEntities.ICE_DRAGON.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
